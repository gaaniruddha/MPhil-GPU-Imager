...
bool gDo_gridding_imaging=true; 
...
int main(int argc,char* argv[])
{
  ...
  // START 
  clock_t stime = clock();
  printf("CLOCK run_imager main() START :");
  ...
  clock_t etime = clock();
  double s = double(etime-stime)/CLOCKS_PER_SEC;
  double ms = s*1000;
  printf("CLOCK run_imager main() END : %.6f [seconds], %.3f [ms]\n",s,ms);
}
